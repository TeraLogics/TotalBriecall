		...
		describe('searchFoodRecalls', function () {
			...
			it('should format inputs and return a valid formatted recall response when inputs are provided', function (done) {
				var obj = {
					...
					expectedSearch = 'distribution_pattern:(VA+Virginia+nationwide+\"national+distribution\"+\"nation+wide\"+nationally+us+usa)+AND+status:ongoing+AND+event_id:12345+AND+recalling_firm:test+AND+recall_initiation_date:[1970-01-01+TO+1970-01-01]+AND+classification:(\"Class+I\"+\"Class+II\")+AND+()';
