...
function _formatRecallResults(data) {
	data.results = _.map(data.results, function (foodrecall) {
		...
		foodrecall.recall_initiation_date = moment.utc(foodrecall.recall_initiation_date, 'YYYY-MM-DD').unix();
		foodrecall.report_date = moment.utc(foodrecall.report_date, 'YYYY-MM-DD').unix();
